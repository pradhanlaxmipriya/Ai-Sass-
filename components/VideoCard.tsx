...
const  VideoCard: React.FC<VideoCardProps> = ({video, onDownload}) => {
    ...
    const handleDownload = async (url: string, title: string) => {
        try {
            // Method 1: Simple download (works for most cases)
    ...
    const handleDownloadClick = async () => {
        ...
        try {
            ...
            // Use parent's onDownload if provided, otherwise use our own
            if (onDownload) {
                onDownload(url, video.title);
            } else {
                await handleDownload(url, video.title);
            }
        } catch (error) {
   ...
   if (!size || isNaN(num)) return "Unknown";
   return filesize(num);
 }, []);
